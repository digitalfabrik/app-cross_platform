...
namespace Integreat.Shared.Test.Services.Loader
{
    ...
    {
        private EventPageLoader _loader;
        ...
        public void BeforeAll()
        {
            ...
            _loader = new EventPageLoader(_language, _location, _persistenceService, _networkService);
        }
        ...
        public async void TestLoadingSavesInDatabase()
        {
            ...
            Assert.AreEqual(new DateTime(), Preferences.LastEventPageUpdateTime(_language, _location));
            Assert.NotNull(_loader);
            var pages = await _loader.Load();
            var updateTimeChanged = Preferences.LastEventPageUpdateTime(_language, _location);
            Assert.AreNotEqual(new DateTime(), updateTimeChanged);
            Assert.NotNull(pages);

            var pages2 = await _loader.Load();
            var updateTimeNotChanged = Preferences.LastEventPageUpdateTime(_language, _location);
