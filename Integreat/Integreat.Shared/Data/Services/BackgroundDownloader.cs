    ...
    public class BackgroundDownloader : IBackgroundLoader
    {
        ...
        private void Worker(Action refreshCommand)
        {
            ...
            foreach (var page in pages)
            {
                ...
                var res = Regex.Replace(page.Content, "https?:\\/\\/(www\\.)?[-a-zA-Z0-9@:%._\\+~#=]{2,256}\\.[a-z]{2,6}\\b([-a-zäöüßA-ZÄÖÜ0-9@:%_\\+.~#?&//=]*)(pdf){1}", PdfWorkAroundReplacer);
                res = Regex.Replace(res, "https?:\\/\\/(www\\.)?[-a-zA-Z0-9@:%._\\+~#=]{2,256}\\.[a-z]{2,6}\\b([-a-zäöüßA-ZÄÖÜ0-9@:%_\\+.~#?&//=]*)(jpg|png|jpeg){1}", UrlReplacer);
        ...
        private string PdfWorkAroundReplacer(Match match)
        {
            CancellationTokenSource.Token.ThrowIfCancellationRequested();
            ...
            var result = $"http://{localPath}.tmpextensions";
            ...
            else
            {
                ...
                catch (Exception e)
                {
                    ...
                    result = $"http://{match.Value}.tmpextensions";
                }
